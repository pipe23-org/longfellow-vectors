...
- The `generator` field of constructed provenance now holds the generating command line with every value the command generated filled in, and re-running it reproduces the bytes for every `generate.py` command but `proof`; previously it named a tool and mode. `admit.py` takes `--ref`, the generator's commit, required with `--generator`. Signatures use the RFC 6979 nonce, key scalars and `IssuerSignedItem` salts derive from a seed, and a seed or certificate serial the command generated is filled into the recorded command line. (#8)
